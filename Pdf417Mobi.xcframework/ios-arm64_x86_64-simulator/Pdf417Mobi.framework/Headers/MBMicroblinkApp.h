...
@interface MBBMicroblinkApp : NSObject
...
static inline NSString * MBB_LOCALIZED_DEFAULT_STRING(NSString *key) {
    return ([NSString stringWithFormat:@"[[%@]]", key]);
}

static inline NSString * MBB_LOCALIZED_FOR_LANGUAGE(NSString *key, NSString *language) {

    NSString *overridenStringFromCustomBundle = NSLocalizedStringWithDefaultValue(key,
                                                                                  language,
                                                                                  [MBBMicroblinkApp sharedInstance].customResourcesBundle,
                                                                                  @"",
                                                                                  @"");
    ...
    return NSLocalizedStringWithDefaultValue(key,
                                             language,
                                             [MBBMicroblinkApp sharedInstance].resourcesBundle,
                                             MBB_LOCALIZED_DEFAULT_STRING(key),
                                             nil);
}

static inline NSString * MBB_LOCALIZED(NSString *key) {
    NSString *localizationFileName = [MBBMicroblinkApp sharedInstance].customLocalizationFileName;
    ...
    NSString *language = [MBBMicroblinkApp sharedInstance].language;
    NSString *localizedString = MBB_LOCALIZED_FOR_LANGUAGE(key, language);
    
#ifndef DNDEBUG
    return localizedString;
#else
    if ([localizedString isEqualToString:MBB_LOCALIZED_DEFAULT_STRING(key)]) {
        if (![language isEqualToString:@"ar-AE"] && [language hasPrefix:@"ar"]) {
            localizedString = MBB_LOCALIZED_FOR_LANGUAGE(key, @"ar-AE");
        }
        else if (![language isEqualToString:@"en"]) {
            localizedString = MBB_LOCALIZED_FOR_LANGUAGE(key, @"en");
        }
    }
    return localizedString;
#endif
